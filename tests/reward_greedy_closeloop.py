...
def run_reward_greedy(args, robot, ACTIONS):
    p_act = np.ones(len(ACTIONS))
    p_norm = p_act / np.sum(p_act)
    ...
    for t in range(max_time):
        action_key = np.random.choice(len(ACTIONS), p=p_norm)
        dP = ACTIONS[action_key]
        currentGripperJointValues = robot.get_gripper_jpos()
        jointValues_target = currentGripperJointValues + dP
        ...
        # update action weights and normalization
        p_act[action_key] += num_broken - prev_broken
        p_act[p_act < 0] = 0
        p_norm = p_act / np.sum(p_act)

        prev_broken = num_broken 
...
def main(args):
    ...
    actions = [act_mag * np.array([1, 0, 0, 0, 0, 0]),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_iterations', default=180, type=int, help='grip force in each action excution')
